const WhatsAppMessageByStatus = [
  {
    status: 0, // Initial confirmation status
    templates: [
      {
        id: "request_confirmation",
        title: "Ditvi Biodata - Request Confirmation",
        message: `Dear Ditvi Biodata Team! 🎉

        This is regarding my Request No. *$$requestNumber$$*.

        I have just submitted my biodata request and would like to know about the next steps.
  ...
  try {
    ...
    const messages = statusTemplates.map((template) => {
      ...
      const replacements = {
        ...
        "\\$\\$requestNumber\\$\\$":
          info?.requestNumber?.toString()?.trim() || "N/A",
        "\\$\\$modelNumber\\$\\$":
          info?.modelNumber?.toString()?.trim() || "N/A",
